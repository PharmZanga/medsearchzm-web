...
const medicines: Medicine[] = [
    ...
    },
    {
        ...
        prescription: true,
        description:
            "Antibiotic medicine requiring pharmacist prescription review.",
    ...
    },
    {
        ...
        prescription: true,
        description:
            "Used for blood pressure control when prescribed by a clinician.",
    ...
    },
    {
        ...
        prescription: true,
        description:
            "Diabetes medicine used as directed by a healthcare provider.",
    ...
    },
    {
        ...
        prescription: true,
        description:
            "Cold-chain insulin product. Prescription and counselling required.",
        pharmacies: ["medlink"],
    },
];
...
const accounts = [
    {
        ...
        icon: UserRound,
        summary:
            "Search, book, buy OTC medicines, save providers and receive notifications.",
        fields: [
            "Full name",
            "NRC or passport",
            "Phone OTP",
    ...
    },
    {
        ...
        icon: Stethoscope,
        summary:
            "Create a professional profile, post education, answer questions and offer consultations.",
        fields: [
            "Profession",
            "Registration number",
            "Specialty",
    ...
    },
    {
        ...
        icon: Building2,
        summary:
            "Manage a facility profile, appointments, workers, services, orders and campaigns.",
        fields: [
            "Facility name",
            "Facility type",
            "License number",
...
const modules = [
    {
        id: "pharmacy" as ModuleId,
        title: "Pharmacies",
        icon: Pill,
        color: "bg-blue-500",
    },
    {
        id: "hospitals" as ModuleId,
        title: "Hospitals",
        icon: Hospital,
        color: "bg-teal-500",
    },
    ...
    {
        id: "community" as ModuleId,
        title: "Community",
        icon: Users,
        color: "bg-sky-500",
    },
    {
        id: "medical-profile" as ModuleId,
        title: "My Medical Profile",
        icon: ShieldCheck,
        color: "bg-emerald-500",
    },
    {
        id: "accounts" as ModuleId,
        title: "Accounts",
        icon: Building2,
        color: "bg-orange-500",
    },
...
function App() {
    ...
    const [loggedInAccount, setLoggedInAccount] = useState<AccountType | null>(
        null,
    );
    ...
    const [actionPanel, setActionPanel] = useState<ActionPanelContent | null>(
        null,
    );
    ...
    const selectedMedicine =
        medicines.find((medicine) => medicine.id === selectedMedicineId) ??
        medicines[0];
    ...
    function navigate(module: ModuleId) {
        ...
        setActionPanel(null);
        setToast(
            `Opened ${module === "home" ? "home" : module.replace("-", " ")}`,
        );
    }
    ...
    function openFacilityWindow(facility: Facility) {
        ...
        const inventory = medicines.filter((medicine) =>
            medicine.pharmacies.includes(facility.id),
        );
        const serviceBookingActions = facility.services.map((service) => ({
            ...
            items: [
                ...
                "Confirm before sending the request to the provider.",
            ],
        ...
        const inventoryPanel = {
            ...
            items: inventory.length
                ? inventory.map(
                      (medicine) =>
                          `${medicine.name} · ${medicine.brand} · ZMW ${medicine.price} · ${medicine.stock} packs`,
                  )
                : [
            ...
            actions: inventory.map((medicine) => ({
                ...
                items: [
                    ...
                    medicine.prescription
                        ...
                        : "Available for OTC checkout.",
                    `Available at ${facility.name}`,
                ],
                primaryAction: medicine.prescription
                    ? "Upload prescription"
                    : "Buy now",
            })),
            primaryAction: inventory.length
                ? "Proceed to checkout"
                : "Back to profile",
        };
        const departmentsPanel = {
            ...
            items: facility.services.map(
                (service) => `${service} · Open for patient request`,
            ),
        ...
        openAction({
            ...
            items: [
                ...
                facility.delivery
                    ? "Delivery available"
                    : "Pickup or appointment request available",
            ],
            actions: [
                {
                    ...
                    items: [
                        ...
                        "Cancel | Call confirmation appears first",
                        "Audio calls use the native phone call flow.",
                    ],
                ...
                },
                {
                    ...
                    items: [
                        "Ask about medicine stock or services",
                        "Upload prescription or image",
                        "Read receipts and typing indicator appear here.",
                    ],
                ...
                },
                {
                    ...
                    items: [
                        `WhatsApp: ${facility.phone}`,
                        `Hello ${facility.name}, I found you on MedSearch Africa and would like assistance.`,
                        "User confirms before leaving MedSearch.",
                    ],
                ...
                },
                {
                    ...
                    items: [
                        `Distance remaining: ${facility.distance}`,
                        "Walking route",
                        "Driving route",
                        "Motorcycle route",
                        "Turn left in 200m, then continue 1km.",
                    ],
                ...
                },
                {
                    ...
                    subtitle:
                        facility.type === "Pharmacy"
                            ? "Search medicines, compare stock and order."
                            : "Departments and services available at this facility.",
                    items: inventory.length
                        ? inventory.map(
                              (medicine) =>
                                  `${medicine.name} · ZMW ${medicine.price} · ${medicine.stock} packs`,
                          )
                        : facility.services.map(
                              (service) =>
                                  `${service} · Open for patient request`,
                          ),
                    actions:
                        facility.type === "Pharmacy"
                            ? inventoryPanel.actions
                ...
                },
                {
                    ...
                    items: [
                        "Service price range shown before booking",
                        "NHIMA/private insurance support shown where available",
                        "Patient confirms before payment or booking.",
                    ],
                    primaryAction: "View prices",
                },
                {
                    title: "Delivery System",
                    subtitle: facility.delivery
                        ? "Delivery available from this provider."
                        : "Delivery can be requested if enabled.",
                    items: facility.delivery
                        ? [
                              "Delivery fee from ZMW 20",
                              "Estimated arrival 45-90 minutes",
                              "Prescription required before delivery when needed",
                              "Track delivery inside MedSearch.",
                          ]
                        : [
                              "Pickup available",
                              "Delivery request can be sent to provider",
                              "Confirmation required before order.",
                          ],
                ...
                },
                {
                    ...
                    items: [
                        `Today: ${facility.hours}`,
                        "Weekend hours shown here",
                        "Holiday closure alerts shown here.",
                    ],
    ...
    function openMedicineWindow(medicine: Medicine) {
        ...
        const stockFacilities = facilities.filter((facility) =>
            medicine.pharmacies.includes(facility.id),
        );
        openAction({
            ...
            items: [
                ...
                medicine.prescription
                    ? "Prescription required before checkout or delivery"
                    : "Available for OTC purchase",
            ...
            return (
                <PharmacyModule
                    facilities={filteredFacilities.filter(
                        (facility) => facility.type === "Pharmacy",
                    )}
...
function SiteFooter() {
    return (
        <footer className="mt-10 border-t border-teal-100 bg-white">
            <div className="mx-auto grid max-w-7xl gap-6 px-4 py-8 text-sm text-slate-600 md:grid-cols-[1fr_auto] md:items-center lg:px-8">
                <div>
                    <strong className="block text-base text-slate-950">
                        MedSearch Africa
                    </strong>
                    <p className="mt-1">
                        Improving access to trusted healthcare information and
                        services across Africa.
                    </p>
                    <p className="mt-2 text-xs">
                        For urgent medical care, contact your local emergency
                        service or visit the nearest health facility.
                    </p>
                </div>
                <nav
                    aria-label="Legal"
                    className="flex flex-wrap gap-4 font-semibold text-[#087D7D]"
                >
    ...
    return (
        <header className="sticky top-0 z-30 border-b border-teal-100 bg-white/90 backdrop-blur">
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 lg:px-8">
                ...
                <button
                    className="rounded-full bg-[#FF8A00] px-4 py-2 text-sm font-bold text-white shadow-sm"
...
function AppPage({
    ...
    return (
        <section className="mx-auto max-w-7xl px-4 py-4 md:py-6 lg:px-8">
            ...
            <div
                className={
                    showCart ? "grid gap-8 lg:grid-cols-[1fr_360px]" : ""
                }
            >
                <div>{children}</div>
    ...
    return (
        <section className="overflow-hidden rounded-[28px] bg-[#1AA6A6] p-6 text-white shadow-sm lg:p-8">
            <div className="grid gap-6 md:grid-cols-[1fr_260px] md:items-center">
                <div>
                    <p className="mb-3 inline-flex rounded-full bg-white/16 px-3 py-1 text-sm font-semibold">
                        MedSearch Africa public beta
                    </p>
                    <h1 className="max-w-2xl text-3xl font-black leading-tight lg:text-5xl">
                        Find healthcare services with confidence.
                    </h1>
                    <p className="mt-4 max-w-xl text-base leading-7 text-white/86">
                        Explore the MedSearch experience for finding providers,
                        medicines and services. Transactional features remain in
                        demonstration mode during the public beta.
                    </p>
                    <div className="mt-6 flex flex-wrap gap-3">
                        <button
                            className="rounded-full bg-white px-5 py-3 font-bold text-[#087D7D]"
                ...
                <div className="rounded-3xl bg-white/14 p-4">
                    <div className="rounded-2xl bg-white p-4 text-slate-950 shadow-lg">
                        <div className="mb-4 flex items-center justify-between">
                            ...
                            <span className="rounded-full bg-[#EAF8F8] px-3 py-1 text-xs font-bold text-[#087D7D]">
                                Interactive
                            </span>
                        </div>
                        <div className="space-y-3">
                            {[
                                "Fine Pharmacy: profile + stock",
                                "Services: facilities + booking",
                                "Community: tabs + actions",
                            ].map((item) => (
                                <div
...
function PharmacyModule({
    ...
    const activeFacility =
        facilities.find((facility) => facility.id === selectedFacility.id) ??
        facilities[0] ??
        selectedFacility;
    const inventory = medicines.filter((medicine) =>
        medicine.pharmacies.includes(activeFacility.id),
    );
    const filteredPharmacies = facilities
        .filter(
            (facility) =>
                ...
                (pharmacyFilter === "Open Now" &&
                    facility.status === "Open Now") ||
                (pharmacyFilter === "Delivery" && facility.delivery) ||
                (pharmacyFilter === "Consultation" &&
                    facility.services
                        .join(" ")
                        .toLowerCase()
                        .includes("consult")) ||
                (pharmacyFilter === "24 Hours" &&
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                ...
                subtitle="Find pharmacies, open profiles, view inventory, chat, call and navigate."
            />
            <LiveFacilityMap
            ...
            />
            <Inventory
                title={`${activeFacility.name} medicines in stock`}
                medicines={inventory}
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                ...
                subtitle="Open facility profiles, services, contact options, appointments and navigation."
            />
            <LiveFacilityMap
    ...
    return (
        <article className="overflow-hidden rounded-3xl border border-teal-100 bg-white shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3 p-5">
                <div>
                    ...
                    <p className="text-sm text-slate-500">
                        Tap a pin or list card to focus the map and open the
                        profile window.
                    </p>
                </div>
                ...
                {facilities.slice(0, 4).map((facility, index) => {
                    ...
                    return (
                        ...
                        >
                            <MapPin
                                className={
                                    isSelected ? "text-white" : "text-[#087D7D]"
                                }
                                size={16}
                            />
            ...
            <div className="grid gap-3 p-5 md:grid-cols-[1fr_auto] md:items-center">
                <div>
                    <p className="text-xs font-black uppercase tracking-wide text-[#087D7D]">
                        Selected on map
                    </p>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                        ...
                        {selectedFacility.status} · {selectedFacility.hours}
                    </p>
                </div>
                <button
                    className="rounded-full bg-[#1AA6A6] px-5 py-3 text-sm font-black text-white"
                    onClick={() => onOpenFacility(selectedFacility)}
                >
                    Open profile window
                </button>
            </div>
                ...
                {facilities.map((facility) => (
                    ...
                    >
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                <p className="text-sm font-bold text-[#087D7D]">
                                    {facility.type} · {facility.province}
                                </p>
                                <h3 className="mt-1 text-xl font-black text-slate-950">
                                    {facility.name}
                                </h3>
                                <p className="mt-2 text-sm text-slate-600">
                                    {facility.distance} away · {facility.rating}
                                    /5 patient rating
                                </p>
                            </div>
                            <span className="rounded-full bg-[#EAF8F8] px-3 py-1 text-xs font-bold text-[#087D7D]">
                        ...
                        <div className="mt-4 flex flex-wrap gap-2">
                            {facility.services.slice(0, 4).map((service) => (
                                <PillTag key={service}>{service}</PillTag>
                            ))}
                        </div>
                    </button>
                ))}
...
function FacilityProfile({
    ...
    const actionPanels = {
        ...
        chat: {
            title: `Chat with ${facility.name}`,
            subtitle:
                "In-app messaging window for pharmacy or facility support.",
            items: [
                "User: Do you have insulin?",
                "Provider: Yes, Humulin and Mixtard are available.",
                "Prescription upload, read receipts and typing indicator enabled.",
            ],
            primaryAction: "Send message",
        },
        directions: {
            title: `Navigate to ${facility.name}`,
            subtitle:
                "In-app map and route guidance, not a forced external redirect.",
            items: [
                `Distance remaining: ${facility.distance}`,
                "Turn left in 200m",
                "Continue 1km",
                "Walking, driving and motorcycle routes available.",
            ],
            primaryAction: "Start navigation",
        },
        book: {
            ...
            items: [
                "Calendar: Today, Tomorrow, Friday",
                "Times: 09:00, 11:30, 14:00",
                "Confirmation prompt appears before final booking.",
            ],
        ...
        rx: {
            ...
            subtitle:
                "Secure prescription upload for review before order or delivery.",
            items: [
                "Upload photo or PDF",
                "Pharmacist reviews medicine request",
                "User receives chat update when approved.",
            ],
        ...
        whatsapp: {
            ...
            items: [
                `WhatsApp number: ${facility.phone}`,
                `Message: Hello ${facility.name}, I found you on MedSearch Africa and would like assistance.`,
                "External WhatsApp opening is optional; the app keeps in-app chat available.",
            ],
            primaryAction: "Open WhatsApp",
        },
        share: {
            title: `Share ${facility.name}`,
            subtitle:
                "Share this provider profile with a patient, caregiver or family member.",
            items: [
    ...
    const profileWindows = [
        ...
        },
        {
            ...
            items: [
                `Phone: ${facility.phone}`,
                `WhatsApp: ${facility.phone}`,
                "Email: support@medsearch.co.zm",
                "After-hours line shown when available.",
            ],
        ...
        },
        {
            ...
            items: [
                ...
                "Motorcycle route available",
                `Turn-by-turn directions to ${facility.name}.`,
            ],
            primaryAction: "Start navigation",
        },
        {
            ...
            subtitle:
                facility.type === "Pharmacy"
                    ? "Search medicines, compare stock and order."
                    : "Clinical services and departments available.",
            items: inventory.length
                ? inventory.map(
                      (medicine) =>
                          `${medicine.name} · ZMW ${medicine.price} · ${medicine.stock} packs`,
        ...
        },
        {
            ...
            subtitle: "Transparent patient-facing costs.",
            items: [
                "Consultation/service price range shown before booking",
                "Insurance and NHIMA support shown where available",
                "Generic vs brand pricing visible for medicines.",
            ],
            primaryAction: "View prices",
        },
        {
            title: "Delivery System",
            subtitle: facility.delivery
                ? "Delivery is available for this provider."
                : "Delivery options depend on this provider.",
            items: facility.delivery
                ? [
                      "Delivery fee from ZMW 20",
                      "Estimated arrival: 45-90 minutes",
                      "Prescription upload required where needed",
                      "Driver tracking appears inside MedSearch.",
                  ]
                : [
                      "Pickup available",
                      "Delivery request can be sent if provider enables it",
                      "User receives confirmation before order.",
                  ],
            primaryAction: "Open delivery",
        },
        {
            ...
            items: [
                `Today: ${facility.hours}`,
                "Weekend hours shown where available",
                "Public holiday closure notices appear here.",
            ],
            primaryAction: "View hours",
        },
        {
    ...
    return (
        <article className="rounded-3xl border border-teal-100 bg-white p-6 shadow-sm">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <p className="text-sm font-bold text-[#087D7D]">
                        {facility.type} profile
                    </p>
                    <h2 className="mt-1 text-2xl font-black text-slate-950">
                        {facility.name}
                    </h2>
                    <p className="mt-2 text-sm text-slate-600">
                        {facility.distance} away · {facility.hours} ·{" "}
                        {facility.rating}/5 rating
                    </p>
                </div>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-4 lg:grid-cols-7">
                <ActionButton
                    icon={Phone}
                    label="Call"
                    onClick={() => onOpenAction(actionPanels.call)}
                />
                <ActionButton
                    icon={MessageCircle}
                    label="Chat"
                    onClick={() => onOpenAction(actionPanels.chat)}
                />
                <ActionButton
                    icon={MessageCircle}
                    label="WhatsApp"
                    onClick={() => onOpenAction(actionPanels.whatsapp)}
                />
                <ActionButton
                    icon={MapPin}
                    label="Directions"
                    onClick={() => onOpenAction(actionPanels.directions)}
                />
                <ActionButton
                    icon={CalendarDays}
                    label="Book"
                    onClick={() => onOpenAction(actionPanels.book)}
                />
                <ActionButton
                    icon={Upload}
                    label="Upload Rx"
                    onClick={() => onOpenAction(actionPanels.rx)}
                />
                <ActionButton
                    icon={ChevronRight}
                    label="Share"
                    onClick={() => onOpenAction(actionPanels.share)}
                />
            </div>
            <div className="mt-5">
                <h3 className="text-lg font-black text-slate-950">
...
function Medicines({
    ...
    const visibleMedicines = medicines.filter(
        (medicine) =>
            (category === "All" || medicine.category === category) &&
            (brand === "All" || medicine.brand === brand),
    );
    const stockFacilities = facilities.filter((facility) =>
        selectedMedicine.pharmacies.includes(facility.id),
    );
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                ...
                subtitle="Search by medicine name, generic name, category or brand such as Denk, Shalina and Yashi."
            />
            <FilterRow
                values={categories}
                active={category}
            ...
            <article className="rounded-3xl border border-teal-100 bg-white p-6 shadow-sm">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-3">
                    <InfoTile
                        label="Generic"
                        value={selectedMedicine.generic}
                    />
                    <InfoTile label="Brand" value={selectedMedicine.brand} />
                    <InfoTile
                        label="Price"
                        value={`ZMW ${selectedMedicine.price}`}
                    />
                </div>
                ...
                <div className="mt-5 flex flex-wrap gap-3">
                    <button
                        className="rounded-full bg-[#1AA6A6] px-5 py-3 font-bold text-white"
                        onClick={() => {
                            ...
                            onOpenAction({
                                ...
                                items: [
                                    ...
                                    `Price: ZMW ${selectedMedicine.price}`,
                                    "Quantity and delivery can be adjusted before checkout.",
                                ],
                        ...
                        onClick={() =>
                            onOpenAction({
                                ...
                                items: [
                                    ...
                                    "Payment confirmation before order is submitted.",
                                ],
        ...
        <article className="h-full rounded-3xl border border-teal-100 bg-white p-5 shadow-sm">
            <div className="flex gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-slate-500">
                        {medicine.generic} · {medicine.category}
                    </p>
            ...
            <div className="mt-4 flex items-center justify-between rounded-2xl bg-slate-50 p-3">
                ...
                <span
                    className={`rounded-full px-3 py-1 text-xs font-bold ${medicine.prescription ? "bg-orange-100 text-orange-700" : "bg-emerald-100 text-emerald-700"}`}
                >
                    {medicine.prescription
            ...
            <button
                className="mt-4 w-full rounded-full bg-[#1AA6A6] px-4 py-3 font-bold text-white"
                onClick={(event) => {
                    event.stopPropagation();
                    onAddToCart(medicine.id);
                    onOpenAction?.({
                        title: medicine.prescription
                            ? `Upload prescription for ${medicine.name}`
                            : `${medicine.name} added to cart`,
                        subtitle: medicine.prescription
                            ? "Prescription review window."
                            : "Cart confirmation window.",
                        items: medicine.prescription
                            ? [
                                  "Upload prescription photo or PDF",
                                  "Pharmacist reviews before checkout",
                                  "Chat opens after review if clarification is needed.",
                              ]
                            : [
...
function Services({
    ...
    const serviceKeywords = selected.includes("telemedicine")
        ? ["telemedicine"]
        : selected.includes("laboratory") ||
            selected.includes("blood") ||
            selected.includes("hiv") ||
            selected.includes("covid") ||
            selected.includes("tb")
          ? ["lab", "blood", "hiv", "covid", "tb", "malaria"]
          : selected.includes("x-ray") || selected.includes("radiology")
            ? ["x-ray"]
            : selected.includes("prescription") ||
                selected.includes("medicine") ||
                selected.includes("chronic") ||
                selected.includes("medication")
              ? ["prescription", "medicine", "chronic", "delivery", "review"]
              : selected.includes("general") ||
                  selected.includes("child") ||
                  selected.includes("maternal")
    ...
    const offeringFacilities = facilities.filter((facility) => {
        ...
        return serviceKeywords.some((keyword) =>
            facilityServices.includes(keyword),
        );
    });
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                title="Medical Services"
                subtitle="Choose a service category, then see facilities offering that service with distance, status and booking actions."
            />
            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                <button
                                    ...
                                    onClick={() => {
                                        ...
                                        onOpenAction({
                                            title: service,
                                            subtitle:
                                                "Service information before choosing a facility.",
                                            items: [
                                                "Short description shown to the patient",
                                                "Price range displayed before facility list",
                                                "Tap a facility to open profile, booking, call or directions.",
                                            ],
            ...
            <article className="rounded-3xl border border-teal-100 bg-white p-6 shadow-sm">
                <h3 className="text-xl font-black">
                    Facilities offering {selectedService}
                </h3>
                <p className="mt-2 text-sm text-slate-600">
                    Price varies by facility. Tap a facility to open its
                    profile.
                </p>
                    ...
                    {offeringFacilities.slice(0, 4).map((facility) => (
                        ...
                        >
                            <span>
                                ...
                                <br />
                                <small>
                                    {facility.distance} · {facility.rating}/5 ·{" "}
                                    {facility.status}
                                </small>
                            </span>
                            <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#087D7D]">
                                Book
                            </span>
                        </button>
                    ))}
...
function Community({
    ...
    const questionCards = [
        [
            "Where can I find insulin near me?",
            "Answered by 2 verified pharmacists",
            "Fine Pharmacy and Medlink Pharmacy have insulin in stock.",
        ],
        [
            "Is cough syrup safe during pregnancy?",
            "Waiting for verified answer",
            "A pharmacist or doctor should review the exact product first.",
        ],
        [
            "Which hospital has X-ray today?",
            "Answered by Medsearch Partner Hospital",
            "X-ray is available today from 08:00 to 18:00.",
        ],
    ];
    const eventCards = [
        [
            "Free Cervical Cancer Screening",
            "UTH Women & Newborn Hospital · June 12",
            "Register",
        ],
        [
            "Ask a Pharmacist Live Q&A",
            "Fine Pharmacy · Friday 19:00",
            "Join Session",
        ],
    ...
    const alertCards = [
        [
            "Cholera Alert",
            "New prevention guidance for selected Lusaka communities",
            "View alert",
        ],
        [
            "Medicine Recall Notice",
            "Check product batch before use",
            "Check batch",
        ],
        [
            "Malaria Surge",
            "Find testing and treatment facilities near you",
            "Find services",
        ],
    ];
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                ...
                subtitle="A healthcare social network for trusted posts, comments, videos, groups, events and alerts."
            />
            <FilterRow
                values={tabs}
                active={activeTab}
                        ...
                        <button
                            ...
                            onClick={() =>
                                onOpenAction({
                                    ...
                                    items: [
                                        answer,
                                        "Only verified health workers and facilities can answer as providers.",
                                        "Patients can like, comment, save and follow the provider.",
                                    ],
                        ...
                        <button
                            ...
                            onClick={() =>
                                onOpenAction({
                                    ...
                                    subtitle: detail,
                                    items: [
                                        "Calendar reminder available",
                                        "Location and provider profile can be opened",
                                        "Registration confirmation appears before saving.",
                                    ],
                        ...
                        <button
                            ...
                            onClick={() =>
                                onOpenAction({
                                    ...
                                    subtitle: detail,
                                    items: [
                                        "Location-based public health notice",
                                        "Trusted provider guidance only",
                                        "Nearby services and saved notification options available.",
                                    ],
                                    primaryAction: action,
                                })
                            }
                        >
                            <p className="text-sm font-black uppercase tracking-wide text-[#FF8A00]">
                                Health alert
                            </p>
                                                ...
                                                onClick={() =>
                                                    onOpenAction({
                                                        ...
                                                        items: item.includes(
                                                            ...
                                                            ? [
                                                                  "Jane: Where can I get this service?",
                                                                  "Fine Pharmacy: Available at our branch.",
                                                                  "Dr Mwansa: Thank you for asking.",
                                                              ]
                                                            : [
                                                                  "Action recorded in the community feed",
                                                                  "User notification updated",
                                                                  "Provider engagement analytics updated.",
                                                              ],
                                                        primaryAction:
                                                            item.includes(
                                                                "Comments",
...
function MedicalProfile({
    ...
    const profileTiles = [
        ...
        [
            "Latest 5 records",
            "Visits, labs, imaging",
            "Synced where consent is available",
        ],
        [
            "SmartCare Folder",
            "National EHR",
            "Retrieve visits, labs, prescriptions and referrals",
        ],
    ];
    ...
    return (
        <section className="space-y-5">
            <SectionHeader
                title="My Medical Profile"
                subtitle="Secure health profile with MedSearch Health ID, vitals, medicines, records, emergency contact and SmartCare folder."
            />
            <div className="rounded-3xl bg-[#1AA6A6] p-6 text-white shadow-sm">
                <p className="text-white/80">
                    Martha Tembo · 34 years · Female
                </p>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                    {[
                        "NHIMA Active",
                        "Phone verified",
                        "SmartCare consent required",
                        "PIN / biometric login",
                    ].map((item) => (
                        <span
                            className="rounded-full bg-white/18 px-3 py-1 text-xs font-bold"
                            key={item}
                        >
                            {item}
                        </span>
                    ...
                    <button
                        ...
                        onClick={() =>
                            onOpenAction({
                                ...
                                items: [
                                    detail,
                                    "PIN or biometric check required for sensitive data",
                                    "Share with provider only after patient consent.",
                                ],
                                primaryAction: title.includes("SmartCare")
                                    ? "Request SmartCare sync"
                                    : "Open details",
                            })
                        }
                    >
            ...
            {!authUser && (
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-slate-500">
                            Verification status appears after signup where
                            required.
                        </p>
                    </div>
                </div>
...
function RoleWorkspace({
    ...
    const workspaces = {
        patient: {
            ...
            restricted: [
                "Cannot post health education",
                "Cannot manage facilities",
                "Cannot advertise services",
            ],
        },
        health_worker: {
            ...
            actions: [
                ...
                "Post education article",
                "Upload health video",
                "Offer teleconsultation",
                "View bookings",
            ],
            restricted: [
                "Cannot manage facility subscriptions",
                "Cannot run facility campaigns",
            ],
        },
        facility: {
            ...
            restricted: [
                "Facility actions require license verification before going live",
            ],
        },
    } satisfies Record<
        AccountType,
    ...
    const routeForAction = (item: string): ModuleId | null => {
        ...
        if (text.includes("hospital") || text.includes("doctor"))
            return "hospitals";
        if (text.includes("medicine") || text.includes("otc"))
            return "medicines";
        if (text.includes("appointment") || text.includes("telemedicine"))
            return "services";
        if (text.includes("community") || text.includes("comment"))
            return "community";
                                ...
                                onClick={() => {
                                    ...
                                    onOpenAction({
                                        ...
                                        items: [
                                            ...
                                            "Show relevant records, forms or approvals",
                                            "Save changes to the correct account role.",
                                        ],
                                        primaryAction: "Open",
                                    });
                                }}
        ...
        <article className="rounded-3xl border border-teal-100 bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
                {["Today", "Tomorrow", "Friday"].map((date) => (
                    <button
                        className={`rounded-2xl p-3 font-bold ${selectedDate === date ? "bg-[#1AA6A6] text-white" : "bg-slate-50"}`}
                        key={date}
                        onClick={() => {
                            ...
                            onOpenAction({
                                ...
                                items: [
                                    "Choose a time next",
                                    "Patient can still change the date before confirmation",
                                    "Facility receives booking only after final confirmation.",
                                ],
                ...
                {["09:00", "11:30", "14:00"].map((time) => (
                    <button
                        className={`rounded-2xl p-3 font-bold ${selectedTime === time ? "bg-[#1AA6A6] text-white" : "bg-[#EAF8F8] text-[#087D7D]"}`}
                        key={time}
                        onClick={() => {
                            ...
                            onOpenAction({
                                ...
                                items: [
                                    ...
                                    "Confirm before sending to facility",
                                    "Reminder notification appears after confirmation.",
                                ],
                ...
                onClick={() =>
                    onOpenAction({
                        ...
                        items: [
                            ...
                            "Facility receives booking request",
                            ...
                            >
                                <div className="flex items-center justify-between gap-2">
                                    <span>
                                        ...
                                        <br />
                                        <small>ZMW {medicine.price} each</small>
                                    </span>
                                    <span className="flex items-center gap-2">
                                        ...
                                        <button
                                            className="rounded-full bg-white p-1"
                                            onClick={() =>
                                                onUpdateCart(item.medicineId, 1)
                                            }
                                        >
                                            <Plus size={14} />
                                        </button>
        ...
        <section className="rounded-3xl border border-teal-100 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
                {actions.map(([label, module, Icon]) => (
                    <button
                        className="rounded-2xl bg-[#EAF8F8] p-4 text-left font-bold text-[#087D7D]"
                        key={label}
    ...
    return (
        <section className="rounded-3xl border border-teal-100 bg-white p-5 shadow-sm">
            ...
            <p className="mt-4 text-sm leading-6 text-slate-600">
                I found {selectedMedicine.name} at{" "}
                {selectedMedicine.pharmacies.length} pharmacies. Nearest open
                provider: {selectedFacility.name}.
            </p>
            <div className="mt-4 flex flex-wrap gap-2">
                <button
                    className="rounded-full bg-[#EAF8F8] px-3 py-2 text-xs font-bold text-[#087D7D]"
                    onClick={() => onNavigate("medicines")}
                >
                    Find medicine
                </button>
...
function OrdersSnapshot({ toast }: { toast: string }) {
    return (
        <section className="rounded-3xl border border-teal-100 bg-white p-5 shadow-sm">
            ...
            <p className="mt-3 rounded-2xl bg-[#EAF8F8] p-3 text-sm font-bold text-[#087D7D]">
                {toast}
            </p>
            <div className="mt-4 space-y-3">
                {[
                    ...
                    <div key={title}>
                        ...
                        <div className="h-2 rounded-full bg-slate-100">
                            <span
                                className="block h-2 rounded-full bg-[#1AA6A6]"
                                style={{ width }}
                            />
                        </div>
                    </div>
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/45 px-3 py-4 backdrop-blur-sm md:items-center">
            <button
                ...
                onClick={onClose}
            />
            <section className="relative max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-[28px] border border-teal-100 bg-white p-5 shadow-2xl md:p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <p className="text-sm font-black uppercase tracking-wide text-[#087D7D]">
                            New window
...
function FilterRow({
    ...
    return (
        <div className="flex flex-wrap gap-2">
            {values.map((value) => (
                <button
                    className={`rounded-full px-4 py-2 text-sm font-bold ${active === value ? "bg-[#1AA6A6] text-white" : "bg-white text-slate-600"}`}
                    key={value}
...
function ActionButton({
    ...
    return (
        <button
            className="inline-flex items-center justify-center gap-2 rounded-full bg-[#EAF8F8] px-4 py-2 text-sm font-bold text-[#087D7D]"
            onClick={onClick}
        >
            <Icon size={16} />
            {label}
